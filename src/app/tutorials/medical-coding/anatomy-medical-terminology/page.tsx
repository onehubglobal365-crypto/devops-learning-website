...
const createModuleNavigationItems = (): Array<{ id: string; title: string; href: string; icon?: string | React.ReactNode; children?: Array<{ id: string; title: string; href: string }> }> => {
  ...
  content.forEach((item, index) => {
    ...
    if (item.type === 'table') {
      ...
      if (headers.length > 0 || dataRows.length > 0) {
        nodes.push(
          <div key={`table-${index}`} className="my-6 overflow-x-auto rounded-lg border border-gray-200">
                ...
                <tr className="bg-gray-100">
                  {headers.map((cell, cellIndex) => (
                    <th
                      ...
                      className="border border-gray-300 px-3 sm:px-4 py-2 text-left font-semibold text-gray-900 whitespace-pre-wrap break-words"
                    >
                      {cell}
                    </th>
                  ))}
                  ...
                  >
                    {row.map((cell, cellIndex) => (
                      <td
                        ...
                        className="border border-gray-200 px-3 sm:px-4 py-2 text-gray-900 whitespace-pre-wrap break-words"
                      >
                        {cell}
                      </td>
                    ))}
            ...
            {item.caption && (
              <p className="mt-2 text-sm text-gray-500 italic text-center">{item.caption}</p>
            )}
          </div>
        );
      }
...
export default function AnatomyMedicalTerminologyPage() {
  ...
  return (
    <TechLayout
      ...
      background="linear-gradient(to bottom right, #dcfce7, #d1fae5, #ccfbf1)"
      sidebarTheme="light"
    >
      <div className="min-h-screen relative px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 sm:mb-16 relative z-10">

          <div className="inline-flex items-center justify-center px-8 py-4 rounded-full bg-gradient-to-r from-pink-200 via-purple-200 to-sky-200 shadow-2xl border-2 border-white/70 mb-6 animate-fade-in-up backdrop-blur-md">
          ...
          <div className="mb-6">
            <span className="inline-block px-6 py-2 rounded-full bg-gradient-to-r from-sky-500 to-blue-600 text-white text-sm font-bold shadow-md tracking-wide">
              PART 1
            </span>
          </div>
        ...
        {currentContent && (
          <section
            id={activeSection}
            className="bg-white shadow-2xl rounded-xl p-4 sm:p-6 lg:p-8 scroll-mt-24 mb-8 border border-gray-100"
            style={{ '--text-primary': '#111827', '--text-secondary': '#4b5563' } as React.CSSProperties}
          >
            {/* Breadcrumb */}
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
              <span>{currentContent.group.title}</span>
              <span>›</span>
              <span className="text-sky-600">{currentContent.section.title}</span>
            </div>

            {/* Section Title */}
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-6 pb-4 border-b border-gray-200">
        ...
        {/* Navigation Buttons */}
        <div className="flex justify-between items-center mt-8 pt-8 border-t border-gray-400/30 mb-16">
          <button
            onClick={goToPreviousSection}
            disabled={!hasPrevious}
            className={`flex items-center gap-2 px-4 py-3 rounded-lg transition-all duration-300 ${hasPrevious
              ? 'bg-gray-800 hover:bg-gray-700 text-white cursor-pointer shadow-lg'
          ...
          >
            ...
            <div className="text-left">
              <div className="text-xs text-white/80">Previous</div>
              {hasPrevious && (
