...
export default function SearchBar({ alwaysOpen = false }: SearchBarProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Escape') {
      if (!alwaysOpen) setIsExpanded(false);
      setQuery('');
      setResults([]);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
        if (!alwaysOpen) setIsExpanded(false);
        // Don't clear query on mobile when clicking outside, just close results dropdown if any?
        // Actually for now let's just keep query but hide results if we want?
        // But the search bar stays open.
        // Let's clear results but keep query text visible in input?
        // Or just clear everything as before but keep Expanded true.
        if (alwaysOpen) {
  ...
  return (
    <div ref={containerRef} className={`relative ${alwaysOpen ? 'w-full' : ''}`} style={{ zIndex: 110 }}>
      {/* Search Input Container */}
      <div
        className={`flex items-center transition-all duration-300 rounded-full ${isExpanded || alwaysOpen
          ? `bg-gray-100 dark:bg-white/10 backdrop-blur-md border border-gray-200 dark:border-white/20 shadow-lg ${alwaysOpen ? 'w-full' : 'w-48 md:w-56'}`
          : 'w-10 h-10 hover:bg-gray-100 dark:hover:bg-white/10'
          }`}
      >
        {/* Search Icon / Button */}
        <button
          onClick={alwaysOpen ? undefined : handleExpand}
          className={`flex items-center justify-center transition-all duration-300 ${isExpanded || alwaysOpen ? 'pl-4 pr-2' : 'w-10 h-10'
