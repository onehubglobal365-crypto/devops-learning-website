...
export default function AlumniScrollingGallery({ hideHeader = false }: AlumniScrollingGalleryProps) {
  ...
  const handleDotClick = (index: number) => {
    ...
    if (alumni[index]) {
      setSelectedAlumni(alumni[index]);
    }

    if (!scrollRef.current || !containerRef.current) return;

    // Standard card width (300px) + gap (24px)
    const itemWidth = 324;
    const totalItems = alumni.length;
    const resetPoint = itemWidth * totalItems;
    ...
    // Target is absolute position of the card in the sequence
    // We target the "second" set (stable range)
    let targetPos = (index * itemWidth) + resetPoint;

    // Adjust to center: subtract half screen, add half card
    targetPos -= (containerWidth / 2) - (itemWidth / 2);

    scrollPositionRef.current = targetPos;
  ...
  return (
    <div className={`relative alumni-gallery ${hideHeader ? 'py-4' : 'py-8'}`}>
      ...
      <div className="relative z-10">
          ...
          >
            {displayAlumni.map((alumniProfile, index) => (
              <div
                ...
                className="flex-shrink-0 group cursor-pointer mx-4 pt-4 pb-4"
                style={{ width: '300px' }}
              ...
              >
                <div className="relative h-full">
                  ...
                  <div
                    className={`absolute inset-0 rounded-bl-[2rem] rounded-br-[5rem] rounded-tr-lg rounded-tl-lg transform transition-all duration-500 ease-out group-hover:rotate-0 group-hover:scale-[1.02] ${index % 3 === 0 ? 'bg-emerald-500 rotate-[-6deg]' :
                  ...
                  {/* Main Card */}
                  <div className={`relative bg-white h-full shadow-lg flex flex-col transition-all duration-500 ease-out group-hover:-translate-y-2 group-hover:scale-[1.02] group-hover:shadow-2xl rounded-bl-[2rem] rounded-br-[5rem] rounded-tr-lg rounded-tl-lg border border-transparent group-hover:border-gray-100 ${(index % alumni.length) === activeIndex ? 'ring-4 ring-[#083D77] ring-offset-2 shadow-[0_0_30px_rgba(8,61,119,0.5)] !scale-105 z-50' : ''}`}>

                    {/* Header Section */}
                    <div className="relative flex items-start pt-4 pl-4">
                      {/* Purple Photo Container - OUTSIDE Positioned */}
                      <div className="absolute -top-6 -left-6 w-28 h-28 bg-[#9F21E3] rounded-2xl overflow-hidden shadow-xl z-10 transform transition-transform duration-500 ease-out group-hover:scale-110 group-hover:rotate-3">
                          ...
                          <div className="w-full h-full">
                            <Image
                              ...
                              onError={(e) => {
                                ...
                                if (parent) parent.innerHTML = `<div class="w-full h-full flex items-center justify-center text-white text-xl font-bold">${alumniProfile.initials}</div>`;
                              }}
                            />
                          </div>
                        ) : (
                          <div className="w-full h-full flex items-center justify-center">
                            <span className="text-white text-2xl font-bold">{alumniProfile.initials}</span>
                          </div>
                        )}
                      </div>

                      {/* Right Info */}
                      <div className="flex-1 pt-4 pl-3 pr-2 ml-24">
                        <h3 className="text-xl font-bold text-[#9F21E3] leading-tight mb-1 transition-colors duration-300 group-hover:text-[#7A16B0]">
                          {alumniProfile.name}
                        </h3>
                        <p className="text-gray-600 text-xs font-medium">
                          Placed at {alumniProfile.company || 'Top Company'}
                        </p>
                        {alumniProfile.package && (
                          <p className="text-[#9F21E3] text-xs font-bold mt-1">
                            Package: {alumniProfile.package}
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="p-6 flex flex-col h-full">
                      <div className="mb-4">
                        <div className="text-[#9F21E3] text-4xl leading-none font-serif opacity-40 mb-1">“</div>
                        <p className="text-gray-900 font-bold text-lg leading-snug">
                          {alumniProfile.testimonial ? (
                            alumniProfile.testimonial.split(' ').slice(0, 10).join(' ') + (alumniProfile.testimonial.split(' ').length > 10 ? '...' : '')
                          ) : "TalentSprint gave me the skills to crack my first interview."}
                        </p>
                      </div>

                      {/* Stars/Google Reviews Footer */}
                      <div className="mt-auto pt-4 flex items-center border-t border-gray-100">
                        <div className="flex items-center gap-1">
                          <span className="text-2xl font-bold text-gray-400 mr-2">G</span>
                          {[1, 2, 3, 4, 5].map((star) => (
                        ...
                        <span className="text-[10px] text-gray-400 font-medium ml-2 mt-1">GOOGLE REVIEWS</span>
                      </div>
                    </div>
        ...
        {/* Pagination Dots */}
        <div className="flex justify-center gap-2 mt-6 mb-2 relative z-10">
          {alumni.map((_, index) => (
            <button
              key={index}
              ...
              style={{
                ...
                border: '1px solid rgba(8, 61, 119, 0.4)'
              }}
