...
export default function Sidebar({ items, onThisPage: _onThisPage, activeSection, setActiveSection, activeSubsection, setActiveSubsection, theme = 'dark' }: SidebarProps) {
  ...
  const renderSidebarItem = (item: SidebarItem, level = 0, parentId?: string) => {
    ...
    const activeClass = theme === 'light'
      ? 'bg-gradient-to-r from-sky-400 via-blue-500 to-indigo-500 text-white shadow-lg shadow-blue-500/30 scale-105'
      : 'bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 text-white border border-white/10 shadow-lg shadow-purple-500/20 scale-105';
        ...
        <div className="flex items-center">
          {hasChildren ? (
            <button
              ...
              className={`flex items-center w-full px-4 py-3 text-sm font-semibold rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-blue-500/50 transform ${active
                ? activeClass
            ...
            <Link
              ...
              className={`flex items-center w-full px-4 py-3 text-sm font-semibold rounded-lg transition-all duration-300 group focus:outline-none focus:ring-2 focus:ring-blue-500/50 transform ${active
                ? activeClass
                : `${inactiveTextClass} ${hoverClass}`
                }`}
            >
