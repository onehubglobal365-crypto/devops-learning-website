...
import { Terminal, Code, Coffee, Database } from 'lucide-react';
...
export default function MultiTerminal({ className = '', activeTech = 'linux', onTechChange }: TerminalProps) {
  const [input, setInput] = useState('');
  const [history, setHistory] = useState<CommandHistory[]>([]);
  // currentTech is now derived from props
  const currentTech = activeTech;
...
-rw-r--r-- 1 sql sql  321 Dec 15 10:20 learning.db`,
  ...
  // Handle technology switching via props or internal click
  useEffect(() => {
    setHistory([]);
    const tech = technologies[currentTech];
    if (tech) {
  ...
  return (
    <div className={`bg-gray-900 rounded-2xl overflow-hidden flex flex-col h-[600px] ${className}`}>
      {/* Technology Switcher */}
          ...
          <div className="flex items-center space-x-4">
            ...
            {Object.values(technologies).map((tech) => (
              <button
                ...
                className={`px-3 py-1 rounded text-sm transition-colors ${currentTech === tech.id
