...
export default function MobileMenu({ isOpen, onClose, triggerRef, isLoggedIn }: MobileMenuProps) {
        ...
        <div className="flex flex-col h-full overflow-y-auto">
          ...
          <div className="flex items-center justify-between p-4 border-b border-[#083D77]/10 bg-white/50 backdrop-blur-sm">
            <Link href="/" onClick={onClose} className="flex items-center gap-3 flex-1 min-w-0">
              <div className="w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center p-1 shrink-0 border border-[#083D77]/10">
                <Image
                  src="/logo_new.jpg"
                  alt="OHG365 Logo"
                  width={32}
            ...
            <div className="mt-4 pt-4 border-t border-[#083D77]/10 space-y-3">
              ...
              {!isLoggedIn ? (
                <Link
                  href="/login"
                  onClick={onClose}
                  className="block px-4 py-3 bg-[#083D77] text-white font-bold rounded-xl hover:bg-[#0a3560] shadow-md hover:shadow-lg transition-all duration-300 min-h-[44px] flex items-center justify-center"
                  role="menuitem"
                >
                  Sign In
                </Link>
